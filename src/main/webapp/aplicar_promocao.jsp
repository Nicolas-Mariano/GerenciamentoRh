...
<html>
    ...
    <body>
        ...
        <form action="controller.do" method="GET">
            <input type="hidden" name="acao" value="AbrirAplicarPromocao"/>
        ...
        <c:if test="${not empty setorSelecionado}">
            <form action="controller.do" method="POST" style="margin-top: 20px;">
                <input type="hidden" name="acao" value="AplicarPromocao"/>
                ...
                <c:if test="${not empty contratos}">
                    <label>3. Novo Nível de Senioridade:</label>
                    <select name="novoNivel" required>
                        <option value="">-- Selecione o Novo Nível --</option>
                        <c:forEach var="n" items="${niveis}">
                            <option value="${n.name()}" <c:if test="${n.name() == novoNivelSelecionado}">selected</c:if>>
                                ${n.rotulo}
                            </option>
                        </c:forEach>
                    </select>

                    <label>4. Tipo de Aumento:</label>
                    ...
                    <input type="submit" value="Aplicar Promoção"/>
                </c:if>
            </form>
        </c:if>
